...
use super::{NewProjectStatus, UpdateProjectStatus, repository};
...
pub async fn one(
    ...
    let data = result.ok_or_else(|| AppError::NotFound("ProjectStatus not found".to_string()))?;
...
pub async fn insert(
    ...
    let data = web::block(move || {
        ...
        repository::insert(&mut conn, project_status).map_err(AppError::from)
    })
    .await??;
...
pub async fn update(
    ...
    let data = web::block(move || {
        ...
        repository::update(&mut conn, id, project_status).map_err(AppError::from)
    })
    .await??;
