...
pub async fn one(
    ...
    let data = result.ok_or_else(|| AppError::NotFound("ProjectAiUsage not found".to_string()))?;
...
pub async fn insert(
    ...
    let data = web::block(move || {
        ...
        repository::insert(&mut conn, project_ai_usage).map_err(AppError::from)
    })
    .await??;
...
pub async fn update(
    ...
    let data = web::block(move || {
        ...
        repository::update(&mut conn, id, project_ai_usage).map_err(AppError::from)
    })
    .await??;
